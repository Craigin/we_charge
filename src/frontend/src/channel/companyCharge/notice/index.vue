  ...
  <div class="xc-person-notice">
    ...
    <div class="section">
      <el-radio-group v-model="printType">
        <el-radio :label="1">管理区所有欠费公司</el-radio>
        <el-radio :label="2">单一公司</el-radio>
      </el-radio-group>
    </div>
...
export default {
  ...
  methods: {
    submitAll (type) {
      ...
      api.listCompanyChargeByZone(newQuery).then(data => {
        ...
        this.$confirm(`共${data.list.length}个欠费房间, 是否继续?`, '提示', {
          ...
          try {
            lodopUtils.printCommonPage('company-notice-charge', noticeRow, type)
          } catch (e) {
    ...
    submit (type) {
      ...
      // console.log('noticeRow', lodopUtils.getHtml('company-notice-charge', noticeRow))
      try {
        lodopUtils.printCommonPage('company-notice-charge', noticeRow, type)
      } catch (e) {
        console.log('printerr', e)
